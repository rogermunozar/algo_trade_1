...
Versión mejorada con LÍMITE DE CAPITAL para evitar over-sizing
"""
import numpy as np
import pandas as pd
...
def apply_smart_exit(df: pd.DataFrame,
                     ...
                     risk_per_trade_pct: float = 0.5,  # % de capital a arriesgar por trade
                     max_position_pct: float = 85.0,  # 🆕 Máximo % de capital para una posición
    ...
    - Límite de posición para no exceder el capital disponible
    ...
    Args:
        ...
        risk_per_trade_pct: % de capital a arriesgar por trade (default: 0.5%)
        max_position_pct: Máximo % de capital para una posición (default: 85%)
    ...
    df['smart_position_limited'] = False  # 🆕 Indica si la posición fue limitada por capital
    ...
    for i in range(n):
        ...
        if not in_trade and entry_col in df.columns:
            ...
            if signal == 1 or (signal == -1 and support_short):
                ...
                if signal == 1:  # LONG
                    ...
                    # 🆕 TP ajustado: debe cubrir stop_distance * tp_multiplier + comisiones
                    # Estimamos comisiones como ~2x el valor de posición * commission_pct
                    # Para simplificar: tp_distance = (stop_distance * tp_multiplier) + buffer por comisiones
                    commission_buffer = stop_distance * 0.2  # Agregar 20% más para cubrir comisiones
                    tp_distance = (stop_distance * tp_multiplier) + commission_buffer
                    tp_est = price + tp_distance
                else:  # SHORT
                    position_type = 'SHORT'
                    stop_distance = atr * atr_mult
                    stop_price = price + stop_distance
                    # 🆕 TP ajustado para SHORT
                    commission_buffer = stop_distance * 0.2
                    tp_distance = (stop_distance * tp_multiplier) + commission_buffer
                    tp_est = price - tp_distance

                # 3. Calcular tamaño de posición basado en RIESGO
                position_size_by_risk = (risk_amount / stop_distance) * leverage

                # 🆕 4. Calcular tamaño MÁXIMO por capital disponible
                max_position_value = current_capital * (max_position_pct / 100)
                max_position_size = max_position_value / price

                # 🆕 5. Tomar el MENOR (no exceder capital disponible)
                position_limited = False
                if position_size_by_risk > max_position_size:
                    position_size = max_position_size
                    position_limited = True
                    stats['positions_limited'] += 1

                    # Calcular riesgo REAL (será menor al deseado)
                    actual_risk = (position_size * stop_distance) / leverage
                else:
                    position_size = position_size_by_risk
                    actual_risk = risk_amount

                # 6. Calcular valor de la posición
                position_value = position_size * price

                # 7. Calcular comisión de entrada
                entry_commission = position_value * (commission_pct / 100)

                # 🆕 8. Estimar comisión de salida (aproximada, se ajusta al cerrar)
                estimated_exit_commission = position_value * (commission_pct / 100)
                total_commissions = entry_commission + estimated_exit_commission

                # 9. Verificar que tengamos suficiente capital
                margin_required = position_value / leverage
                total_required = margin_required + entry_commission

                if total_required > current_capital:
                    # No hay suficiente capital (esto no debería pasar con el límite)
                    if log_trades:
                        logger.warning(
                            f"⚠️ Capital insuficiente: Requerido ${total_required:.2f}, Disponible ${current_capital:.2f}")
                ...
                df.at[i, 'smart_capital'] = current_capital
                df.at[i, 'smart_position_limited'] = position_limited
                ...
                if log_trades:
                    ...
                    logger.info(f"  📊 Riesgo deseado: ${risk_amount:.2f} ({risk_per_trade_pct}%)")

                    if position_limited:
                        logger.info(f"  ⚠️  POSICIÓN LIMITADA por capital disponible")
                        logger.info(f"  📊 Riesgo REAL: ${actual_risk:.2f} ({actual_risk / initial_capital * 100:.2f}%)")

                    logger.info(f"  📏 Tamaño posición: {position_size:.6f} unidades")
                    logger.info(
                        f"  💵 Valor posición: ${position_value:,.2f} ({position_value / initial_capital * 100:.1f}% del capital inicial)")
                    logger.info(f"  ⚡ Apalancamiento: {leverage}x")
                    logger.info(
                        f"  💸 Comisiones estimadas: ${total_commissions:.2f} (entrada: ${entry_commission:.2f} + salida: ~${estimated_exit_commission:.2f})")
                    logger.info(f"  🎯 Entrada: ${entry_price:.2f}")
                    logger.info(f"  🛡️  Stop Loss: ${trailing_stop:.2f} (-${stop_distance:.2f})")
                    logger.info(f"  🎯 Take Profit: ${tp_est:.2f} (+${tp_distance:.2f}, incluye buffer de comisiones)")
        ...
        # ========== GESTIÓN DE TRADE ACTIVO ==========
            ...
            if exit_type:
                ...
                stats['total_commission_paid'] += exit_commission
                ...
                if log_trades:
                    ...
                    logger.info(
                        f"  💸 Comisiones totales: ${total_commission:.2f} (entrada: ${entry_commission:.2f} + salida: ${exit_commission:.2f})")
    ...
    if log_trades and stats['total'] > 0:
        ...
        logger.info(f"  └─ Posiciones limitadas: {stats['positions_limited']}")
        logger.info(f"")
        logger.info(f"💵 FINANCIERO:")
        logger.info(f"  ├─ P&L Total: ${stats['total_pnl_usd']:+,.2f}")
        logger.info(f"  └─ Comisiones totales: ${stats['total_commission_paid']:,.2f}")
        logger.info(f"")
        logger.info(f"🔒 Señales ignoradas (durante trades): {stats['signals_ignored']}")
        logger.info(f"{'=' * 70}\n")
...
def calcular_metricas_trailing(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    positions_limited = exits['smart_position_limited'].sum() if 'smart_position_limited' in exits.columns else 0
    ...
    return {
        ...
        'positions_limited': positions_limited,  # 🆕
    }
...
def generar_reporte_trades(df: pd.DataFrame, filename: Optional[str] = None) -> pd.DataFrame:
    ...
    reporte = pd.DataFrame({
        ...
        'position_size': exits['smart_position_size'],
        'position_value': exits['smart_position_value'],
        'pnl_usd': exits['smart_trade_pnl_usd'],
        'pnl_pct': exits['smart_trade_pnl_pct'],
        'tipo_salida': exits['smart_exit_signal'].map({-1: 'STOP', 1: 'TP'}),
        'break_even_activado': exits['smart_break_even'],
        'position_limited': exits.get('smart_position_limited', False),  # 🆕
        'commission_paid': exits['smart_commission_paid']
    })
